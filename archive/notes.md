...
| Contact badges | Kept, converted to text links | Six of the eight were commented-out `img.shields.io` badges. Plain text links carry the same information, cost no external requests, and are readable by screen readers without relying on badge alt text. The commented-out destinations are preserved as a comment in the new README so they can be re-enabled. |
